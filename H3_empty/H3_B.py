...
def initializer_circuit(num_qubits, quantum_vector1, quantum_vector2):
    quantum_vector = 1/np.sqrt(2) * (np.kron(quantum_vector1, np.array([1,0])) + np.kron(quantum_vector2, np.array([0,1])))
...
def compute_q_inner_product(a, b): #5p
    ...
    for _ in range(num_shots):
        pass
        # TODO: count how often the states |0> and |1> were measured for the qubit q_0
        # TODO:    (Hint: qubit q_0 has the index -1 in the sample returned by the measure function)

    p = None # TODO: calculate the probability to measure state |0> for qubit q_0
...
def QKernelMatrix(X1, X2): #2p
    ...
    for i, x1 in enumerate(X1):
        for j, x2 in enumerate(X2):
            qkernel_matrix[i, j] = None # TODO: complete the calculation of the kernel matrix
...
def main(): #3p
    ...
    q_svm = None # TODO: initialize the Support Vector Machine with the quantum kernel
    # TODO: train the q_svm
